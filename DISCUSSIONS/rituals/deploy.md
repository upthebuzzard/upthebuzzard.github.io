...
- [ ] **Displaced pages** — any `.md` file at the repo root other than `index.md`, `404.md`, and `README.md` is displaced: it belongs in `_pages/`. List the offenders (Bash: `ls -1 *.md 2>/dev/null | grep -vx -e index.md -e 404.md -e README.md`) and stop: tell the user which files to move into `_pages/`.
